...
- By default, sub-agents are not allowed to call `sessions_spawn` (`maxSpawnDepth: 1`). If you set `agents.defaults.subagents.maxSpawnDepth >= 2`, depth‑1 sub-agents can spawn depth‑2 children (orchestrator pattern). See [Nested Sub-Agents](/tools/subagents#nested-sub-agents).
